...
def get_ext_ampld_1by1(list_CSF,list_SOMO_DMO,list_sym_sign,list_list_ia_internal,list_list_ia_external,l_axial_sym,debug=False):
        ...
        if l_axial_sym and list_sym_sign[iCSF] != 0:
           ...
           #The pairs in list_ia_int and list_regroupped_ia may not be identical. It is OK for the excitations between deg. shells.
            ...
            UCSF, Uvec, list_pair_ex_space, Umat, list_genmat = \
              prepare_UCSF_for_one_CSF(CSF,list_ia_int,list_zero_theta,False)
    ...
    for iCSF in range(nCSF):
        ...
        if l_axial_sym and list_sym_sign[iCSF] != 0:
            ...
            for ia_pair in grouped_ia_pairs:
                l_ia_exclud = False
                for pair in ia_pair:
                    if pair[0] in list_mo_exclud or pair[1] in list_mo_exclud: l_ia_exclud = True
                if l_ia_exclud:
               ...
               #   #sys.exit()
                if len(list_pair_ex_space1) == 1: 
                    print(f'{ia_pair} does not create any excited CSFs. Now jummping to the next pair.')
...
def generate_CASCI_space(Norb_total, Nel_total,Nactorb,Nactel,S_by2,list_seniority=[],list_mo_exclud=[],l_pair_ex=True,debug=False):
    """
    ...
    if debug: print(f'list_seniority: {list_seniority}')
   #for N_SOMO in range(N_SOMO_min, N_SOMO_max+1,2):
   ...
   #print(list_CSF[-1])
    n_CSF = len(list_CSF)
    if debug: print(f'Total # of CSF states: {n_CSF}')
...
def make_Uvec_full(list_list_ia,list_list_theta,l_use_decomp_genmat,list_list_genmat,list_UCSF_subspace_start_end,list_list_pairex_CSF_vec,list_list_pair_ex_space,debug=False):
    """
    Given the ia pairs, their therta, and the genmats in input, produce the Uvec for the reference CSF
...
def construct_Hmat_CSFs_paral_triu(list_CSF,Enuc,obt,tbt,nparal=1,lrdm=False):

    ndim_CSF = len(list_CSF)
    Hmat_CSF = np.zeros([ndim_CSF,ndim_CSF])

    i_upper = np.triu_indices(ndim_CSF)
   #print(f'Upper diagonal indices including diagonal: {i_upper}')
    i_upper_list = []
    ...
    if lrdm:
        list_list_rdm1 = []
        list_list_rdm2 = []
   #for ind_braket in i_upper_list:
   #    output = Helm_for_one_pair_braket(ind_braket,list_CSF,Enuc,obt,tbt,lrdm)
   #    if lrdm:
   ...
   #        Hmat_CSF[ind_braket[0],ind_braket[1]] = output
    ...
    list_outcome = Parallel(n_jobs=nparal)(delayed(Helm_for_one_pair_braket)(ind_braket,list_CSF,Enuc,obt,tbt,lrdm) for ind_braket in i_upper_list)

    for ii, ind_braket in enumerate(i_upper_list):
