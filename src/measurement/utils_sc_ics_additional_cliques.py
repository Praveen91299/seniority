...
from seniority.src.measurement.utils_partitioning import is_fc_hamiltonian
...
def calculate_cov_dict(state, Hbin, Nqubits):
    ...
    for i, term1 in enumerate(Hbin.binary_terms):
        Op1 = of.get_sparse_operator(of.QubitOperator(term1.to_pauli_strings().key_openfermion()), Nqubits)
        for j, term2 in enumerate(Hbin.binary_terms):
            if i >= j and term1.commute(term2):
                Op2 = of.get_sparse_operator(of.QubitOperator(term2.to_pauli_strings().key_openfermion()), Nqubits)
                cov = of.expectation(Op1 @ Op2, state) - of.expectation(Op1, state) * of.expectation(Op2, state)
...
class OverlappingGroups:
    ...
    @classmethod
    def init_from_binary_terms(cls, terms, condition='fc', n_additional=0, scaling_factor=0.1):
        '''
        ...
        n_additional   : how many additional random maximal cliques to include
        scaling_factor : x 10 is how many additional cliques a pauli should be included it to remove it from the original clique it is in
        '''
        ...
        N_nogroups = len(nonoverlapping_groups)
        terms_c    = terms.copy()
        for alpha in range(n_additional):
            random.shuffle(terms_c)
            
            # do random-SI iteration to obtain a QWC/FC group

            scaling = scaling_factor * n_additional

            new_group = []
            for term in terms_c:
                success = False
                if new_group == []:
                    new_group.append(term.term_w_coeff(term.coeff / scaling))
                    success = True
                else:
                    if term_commutes_with_group(term, new_group, condition):
                        new_group.append(term.term_w_coeff(term.coeff / scaling))
                        success = True
                
                if success:
                    for group in nonoverlapping_groups:
                        group_tuples = [x.binary_tuple() for x in group]
                        if term.binary_tuple() in group_tuples:
                            idx = group_tuples.index(term.binary_tuple())
                            group[idx] = term.term_w_coeff(group[idx].coeff - (term.coeff / scaling))
                            break
            
            # modify three inputs to cls using this new group
            
            for term in new_group:
                term_foo = term.term_w_coeff(0.0)
                if term_foo in overlapping_terms:
                    location = overlapping_terms.index(term_foo)
                    term_exists_in[location].append(N_nogroups + alpha)
                else:
                    overlapping_terms.append(term_foo.term_w_coeff(0.0))
                    term_exists_in.append([N_nogroups + alpha])
            
            nonoverlapping_groups.append(new_group)
