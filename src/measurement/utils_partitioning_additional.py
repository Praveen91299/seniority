...
from seniority.src.measurement.utils_sc_ics_additional_cliques import (
...
def compute_SI_ICS_decomposition(H, state, Nqubits, fragment_type='fc', n_iter=5, n_additional=0, scaling_factor=0.0):
    Hbin                  = convert_QubitOperator_to_BinaryHamiltonian(H)
    cov_dict              = calculate_cov_dict(state, Hbin, Nqubits)
    decomp_obj            = OverlappingGroups.init_from_binary_terms(Hbin.binary_terms, fragment_type, n_additional=n_additional, scaling_factor=scaling_factor)
